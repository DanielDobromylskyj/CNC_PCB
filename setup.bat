...
py -c "import shutil, site, os; src=os.path.join(site.getsitepackages()[0], 'lib', 'site-packages', 'pygame'); dest=os.getcwd(); shutil.copytree(src, os.path.join(dest, 'pygame'), dirs_exist_ok=True) if os.path.exists(src) else print(f'Folder not found: ', src)"
